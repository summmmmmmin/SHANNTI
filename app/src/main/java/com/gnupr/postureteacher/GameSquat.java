...
public class GameSquat extends AppCompatActivity implements TextToSpeech.OnInitListener{
    ...
    class ThreadClass extends Thread {
        ...
        public void run() {
            ...
            if (sideTotalResult[1] && sideTotalResult[0] && currentTime - lastCountTime >= COUNT_RESET_TIME) {
                countNum+=1;
                Toast.makeText(GameSquat.this, "횟수 "+countNum, Toast.LENGTH_SHORT).show();
                lastCountTime = currentTime;
            }

            if (countNum == 5) {
                mediaPlayer2 = MediaPlayer.create(GameSquat.this, R.raw.exercise_finish); // 효과음 리소스 파일 지정
                mediaPlayer2.setLooping(false); // 반복 재생 설정 제거
                mediaPlayer2.start();

                Intent intent = new Intent(GameSquat.this, GamePlayActivity.class);
