...
public class GameBalance extends AppCompatActivity implements TextToSpeech.OnInitListener{
    ...
    class ThreadClass extends Thread {
        ...
        @Override
        public void run() {
            long startTime = System.currentTimeMillis(); // 스레드 시작 시간 기록
            ...
            if (elapsedTime >= 10000) { // 10초가 경과한 경우
                // 10초가 지났으므로 finish() 호출
                mediaPlayer2 = MediaPlayer.create(GameBalance.this, R.raw.exercise_finish); // 효과음 리소스 파일 지정
                mediaPlayer2.setLooping(false); // 반복 재생 설정 제거
                mediaPlayer2.start();

                Intent intent = new Intent(GameBalance.this, GamePlayActivity.class);
    ...
    public void getLandmarksAngleResult(int side) { //0=왼쪽, 1=오른쪽
        if (OutOfRangeSave[23 + side] == true && OutOfRangeSave[25 + side] == true && OutOfRangeSave[27 + side] == true) { //범위 판별 // 30 120
            angleCalculationResult(23 + side, 25 + side, 27 + side, 10f, 410f,3); //90f 120f | 70f 140f
            //발목-무릎-엉덩이 무릎각도
