...
public class BalanceAnalysisActivity extends AppCompatActivity implements TextToSpeech.OnInitListener{
    private TextToSpeech tts;
    private static final long INITIAL_DELAY = 5000;

    private static final String TAG = "BalanceAnalysisActivity";
    ...
    private ImageView balancehead;
    private ImageView balancearm;
    private ImageView balancespine;
    private ImageView balanceleg;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getTimeIntent();
        balancehead= findViewById(R.id.headimage);
        balancearm= findViewById(R.id.armimage);
        balancespine= findViewById(R.id.spineimage);
        balanceleg= findViewById(R.id.legimage);

        headtext = findViewById(R.id.balancehead);
        spinetext = findViewById(R.id.balancespine);
        armtext = findViewById(R.id.balancearm);
        legtext = findViewById(R.id.balanceleg);
    ...
    public void getLandmarksAngleResult(int side) { //0=왼쪽, 1=오른쪽
        ...
        if (OutOfRangeSave[23] == true && OutOfRangeSave[25] == true && OutOfRangeSave[27] == true) { //범위 판별
            ...
            if (markResult[23][25][27] == true) { //각도 판별
                balanceleg.setImageResource(R.drawable.balance_leg_green);
                resultPosture[0] = 2;
                legtext.setText("올바른 자세입니다.");
            } else {
                balanceleg.setImageResource(R.drawable.balance_leg_red);
                resultPosture[0] = 1;
                float angle = calculateAngle(23, 25, 27);
                if (angle < 30f) {
                    legtext.setText("발을 무릎과 조금 더 가까운 위치로 내려주세요.");
                } else if (angle >= 120f) {
                    legtext.setText("발을 무릎과 조금 더 가까운 위치로 올려주세요.");
                }
            }
        } else {
            //여기에 비감지(회색)
            balanceleg.setImageResource(R.drawable.balance_leg_gray);
        ...
        if (OutOfRangeSave[8] == true && OutOfRangeSave[24] == true && OutOfRangeSave[26] == true) { //범위 판별
            ...
            if (markResult[8][24][26] == true) { //각도 판별
                balancespine.setImageResource(R.drawable.balance_spine_green);
                resultPosture[1] = 2;
                spinetext.setText("올바른 자세입니다.");
            } else {
                balancespine.setImageResource(R.drawable.balance_spine_red);
                resultPosture[1] = 1;
                spinetext.setText("몸이 올바르게 정면을 향하도록 해주세요.");
            }
        } else {
            //여기에 비감지(회색)
            balancespine.setImageResource(R.drawable.balance_spine_gray);
        ...
        if (OutOfRangeSave[7 + side] == true && OutOfRangeSave[23 + side] == true) { //범위 판별
            if (!Double.isNaN(getLandmarksAngleTwo(bodyMarkPoint[33 + side], bodyMarkPoint[7 + side], bodyMarkPoint[30 + side], 'x', 'y'))) {
                ...
                if (markResult[7 + side][7 + side][23 + side] == true) { //각도 판별
                    balancehead.setImageResource(R.drawable.balance_head_green);
                    resultPosture[2] = 2;
                    headtext.setText("올바른 자세입니다.");
                } else {
                    balancehead.setImageResource(R.drawable.balance_head_red);
                    resultPosture[2] = 1;
                    headtext.setText("머리를 바로 세우고 정면을 봐주세요.");
                }
            }
            //어깨-귀-귀너머(x+300)
        } else {
            //여기에 비감지(회색)
            balancehead.setImageResource(R.drawable.balance_head_gray);
        ...
        if (OutOfRangeSave[21 + side] == true && OutOfRangeSave[13 + side] == true && OutOfRangeSave[12 + side] == true) { //범위 판별
            ...
            if (markResult[21 + side][13 + side][12 + side] == true) { //각도 판별
                balancearm.setImageResource(R.drawable.balance_arm_green);
                resultPosture[3] = 2;
                armtext.setText("올바른 자세입니다.");
            } else {
                balancearm.setImageResource(R.drawable.balance_arm_red);
                resultPosture[3] = 1;
                armtext.setText("양팔이 일직선을 이루도록 해주세요.");
            }
        } else {
            //여기에 비감지(회색)
            balancearm.setImageResource(R.drawable.balance_arm_gray);
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    protected int getContentViewLayoutResId() {
        return R.layout.activity_balance_analysis;
    }
