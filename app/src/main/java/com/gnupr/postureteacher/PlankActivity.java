...
public class PlankActivity extends AppCompatActivity implements TextToSpeech.OnInitListener{
                        ...
                        timer.schedule(new TimerTask() {
                            ...
                            public void run() {
                                ...
                                uiHandler.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        tts.speak("플랭크 운동이 끝났습니다.", TextToSpeech.QUEUE_FLUSH, null, "endExercise");
...
//                            Toast.makeText(getApplicationContext(), "자세가 불안정한 상태입니다.", Toast.LENGTH_SHORT).show();
    ...
    private void startDialog() {
        ...
        // GIF 파일 설정
        Glide.with(this).load(R.drawable.plankgif).into(gifImageView);
        textView.setText("엎드린 채 팔뚝으로 중심을 잡으세요.\n 이때 몸은 전체에 걸쳐 일자를 유지하세요.");
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (msgDlg != null && msgDlg.isShowing()) {
                    msgDlg.dismiss();
                    tts.speak("플랭크를 시작합니다. 자세를" + timee + "초 유지하세요", TextToSpeech.QUEUE_FLUSH, null, null);
