...
public class PlankAnalysisActivity extends AppCompatActivity implements TextToSpeech.OnInitListener{
    ...
    private ImageView plankhead;
    private ImageView plankarm;
    private ImageView plankspine;
    private ImageView plankleg;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getTimeIntent();
        plankhead= findViewById(R.id.headimage);
        plankarm= findViewById(R.id.armimage);
        plankspine= findViewById(R.id.spineimage);
        plankleg= findViewById(R.id.legimage);

        headtext = findViewById(R.id.plankhead);
        spinetext = findViewById(R.id.plankspine);
        armtext = findViewById(R.id.plankarm);
        legtext = findViewById(R.id.plankleg);
    ...
    public void getLandmarksAngleResult(int side) { //0=왼쪽, 1=오른쪽
        ...
        if (OutOfRangeSave[11 + side] == true && OutOfRangeSave[13 + side] == true && OutOfRangeSave[15 + side] == true) { //범위 판별
            ...
            if (markResult[11 + side][13 + side][15 + side] == true) { //각도 판별
                plankarm.setImageResource(R.drawable.plank_arm_green);
                resultPosture[0] = 2;
                armtext.setText("올바른 자세입니다.");
            } else {
                plankarm.setImageResource(R.drawable.plank_arm_red);
                resultPosture[0] = 1;
                float angle = calculateAngle(11 + side, 13 + side, 15 + side);
                if (angle < 60f) {
                    armtext.setText("팔이 직각이 되도록 조금 더 펴주세요.");
                } else if (angle >= 120f) {
                    armtext.setText("팔이 직각이 되도록 조금 더 굽혀주세요.");
                }
            }
        } else {
            //여기에 비감지(회색)
            plankarm.setImageResource(R.drawable.plank_arm_gray);
        ...
        if (OutOfRangeSave[7 + side] == true && OutOfRangeSave[11 + side] == true && OutOfRangeSave[23 + side] == true) { //범위 판별
            ...
            if (markResult[7 + side][11 + side][23 + side] == true) { //각도 판별
                plankspine.setImageResource(R.drawable.plank_spine_green);
                resultPosture[1] = 2;
                spinetext.setText("올바른 자세입니다.");
            } else {
                plankspine.setImageResource(R.drawable.plank_spine_red);
                resultPosture[1] = 1;
                float angle = calculateAngle(7 + side, 11 + side, 23 + side);
                if (angle < 150f) {
                    spinetext.setText("엉덩이를 조금 더 내려주세요.");
                } else if (angle >= 210f) {
                    spinetext.setText("엉덩이를 조금 더 들어주세요.");
                }
            }
        } else {
            //여기에 비감지(회색)
            plankspine.setImageResource(R.drawable.plank_spine_gray);
        ...
        if (OutOfRangeSave[7 + side] == true && OutOfRangeSave[11 + side] == true) { //범위 판별
            if (!Double.isNaN(getLandmarksAngleTwo(bodyMarkPoint[33 + side], bodyMarkPoint[7 + side], bodyMarkPoint[11 + side], 'x', 'y'))) {
                ...
                if (markResult[7 + side][7 + side][11 + side] == true) { //각도 판별
                    plankhead.setImageResource(R.drawable.plank_head_green);
                    resultPosture[2] = 2;
                    headtext.setText("올바른 자세입니다.");
                } else {
                    plankhead.setImageResource(R.drawable.plank_head_red);
                    resultPosture[2] = 1;
                    headtext.setText("머리와 척추가 일직선 상에 있도록 해주세요.");
                }
            }
            //어깨-귀-귀너머 머리각도(x+300)
        } else {
            //여기에 비감지(회색)
            plankhead.setImageResource(R.drawable.plank_head_gray);
            markResult[7 + side][7 + side][11 + side] = true;
            resultPosture[2] = 0;
        ...
        if (OutOfRangeSave[23 + side] == true && OutOfRangeSave[25 + side] == true && OutOfRangeSave[27 + side] == true) { //범위 판별
            ...
            if (markResult[23 + side][25 + side][27 + side] == true) { //각도 판별
                plankleg.setImageResource(R.drawable.plank_leg_green);
                resultPosture[3] = 2;
                legtext.setText("올바른 자세입니다.");
            } else {
                plankleg.setImageResource(R.drawable.plank_leg_red);
                resultPosture[3] = 1;
                legtext.setText("다리가 일자가 되도록 무릎을 펴주세요.");

            }
        } else {
            //여기에 비감지(회색)
            plankleg.setImageResource(R.drawable.plank_leg_gray);
        ...
        if (markResult[11 + side][13 + side][15 + side] && markResult[7 + side][11 + side][23 + side]
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    protected int getContentViewLayoutResId() {
        return R.layout.activity_plank_analysis;
    }
