...
public class SquatActivity extends AppCompatActivity implements TextToSpeech.OnInitListener{
    ...
    private int squatTargetCount = 2;
    //플랭크할 횟수
    private int squatCurrentCount = 0;
    //현재 플랭크 횟수
        ...
        public void run() {
            ...
            if (wasKneeAngleSatisfied && !isKneeAngleSatisfied) {
                ...
                if (countNum == totalCount) {
                    tts.speak("스쿼트 횟수 " + countNum + "입니다. 운동을 종료합니다.", TextToSpeech.QUEUE_FLUSH, null, UTTERANCE_ID_COMPLETE);
...
//                                                                        Toast.makeText(SquatActivity.this, "새로운 점수가 성공적으로 저장되었습니다. Time : 6, Kcal : 6", Toast.LENGTH_SHORT).show();
                    ...
                    tts.speak("스쿼트 횟수 " + countNum + "입니다", TextToSpeech.QUEUE_FLUSH, null, null);
                }
            }
            // 이전 상태를 현재 상태로 업데이트
...
//////////////////////////////////////////////////////////////////////////////////
    ...
    TimerTask timerTask = new TimerTask() {
        ...
        public void run() {
            ...
            if(!pauseTimerCheck) {
                ...
                nowTime = text_minute + ":" + text_second + " | " + squatCurrentCount + "/" + squatTargetCount + "세트\n(" + spareTime + "% / " + tempTime + "pt)";
            }

            if (timer_minute == 0 && timer_second == 0) {
                if(finalStopCheck == 0) {
                    timer_second += squatBreakTime;
                    //플랭크 쉬는 시간 설정
                    finalStopCheck = 1;
                    breakTimeCheck = true;
                }
                else if(finalStopCheck == 1) {
                    if (squatTargetCount <= squatCurrentCount) {
                        timer_second += 3;
                        finalStopCheck = 2;
                    }
                    else {
    ...
    private void startDialog() {
        ...
        AlertDialog.Builder msgBuilder = new AlertDialog.Builder(SquatActivity.this);
        LayoutInflater inflater = getLayoutInflater();
        View dialogView = inflater.inflate(R.layout.custom_dialog_layout, null);
        ImageView gifImageView = dialogView.findViewById(R.id.gifImageView);

        // GIF 파일 설정
        Glide.with(this).load(R.drawable.squatgif).into(gifImageView);
        // 나머지 메시지 설정
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (msgDlg != null && msgDlg.isShowing()) {
                    msgDlg.dismiss();
                    tts.speak("운동을 시작합니다. 스쿼트를" + timee +"회 하세요.", TextToSpeech.QUEUE_FLUSH, null, null);
                }
            }
        }, 3500); // 5초 후에 닫기
        msgDlg.show();
...
////////////////////////////////////////////////////////////////////////////////
    ...
    public void angleCalculationResult(int firstPoint, int secondPoint, int thirdPoint, float oneAngle, float twoAngle, int callIndex) {
...
/////////////////////////////////////////////////////////////////////////////////
    ...
    public void getLandmarksAngleResult(int side) { //0=왼쪽, 1=오른쪽
        ...
        if (OutOfRangeSave[11 + side] == true && OutOfRangeSave[23 + side] == true && OutOfRangeSave[25 + side] == true) { //범위 판별
            angleCalculationResult(11 + side, 23 + side, 25 + side, 70f, 110f,1); //90f 120f | 70f 140f | 80f 130f

            //무릎-엉덩이-허리
            if (markResult[11 + side][23 + side][25 + side] == true) { //각도 판별
                iv1.setImageResource(R.drawable.squat_spine_green);
                resultPosture[0] = 2;
            } else {
                iv1.setImageResource(R.drawable.squat_spine_red);
                resultPosture[0] = 1;
            }
        } else {
            //여기에 비감지(회색)
            iv1.setImageResource(R.drawable.squat_spine_gray);
        ...
        if (OutOfRangeSave[11 + side] == true && OutOfRangeSave[13 + side] == true && OutOfRangeSave[15 + side] == true) { //범위 판별
            angleCalculationResult(11 + side, 13 + side, 15 + side, 150f, 210f,2); //140f 180f | 120f 180f X //90f 120f
            //어깨-팔꿈치-손목
            if (markResult[11 + side][13 + side][15 + side] == true) { //각도 판별
                iv3.setImageResource(R.drawable.squat_arm_green);
                resultPosture[1] = 2;
            } else {
                iv3.setImageResource(R.drawable.squat_arm_red);
                resultPosture[1] = 1;
            }
        } else {
            //여기에 비감지(회색)
            iv3.setImageResource(R.drawable.squat_arm_gray);
        ...
        if (OutOfRangeSave[7 + side] == true && OutOfRangeSave[11 + side] == true) { //범위 판별
            if (!Double.isNaN(getLandmarksAngleTwo(bodyMarkPoint[33 + side], bodyMarkPoint[7 + side], bodyMarkPoint[11 + side], 'x', 'y'))) {
                ...
                if (markResult[7 + side][7 + side][11 + side] == true) { //각도 판별
                    iv4.setImageResource(R.drawable.squat_head_green);
                    resultPosture[2] = 2;
                } else {
                    iv4.setImageResource(R.drawable.squat_head_red);
                    resultPosture[2] = 1;
                }
            }
            //어깨-귀-귀너머 머리각도(x+300)
            ...
            //여기에 비감지(회색)
            iv4.setImageResource(R.drawable.squat_head_gray);
            markResult[7 + side][7 + side][11 + side] = true;
            resultPosture[2] = 0;
        }
        ...
        if (OutOfRangeSave[23 + side] == true && OutOfRangeSave[25 + side] == true && OutOfRangeSave[27 + side] == true) { //범위 판별 // 30 90
            angleCalculationResult(23 + side, 25 + side, 27 + side, 20f, 110f,3); //90f 120f | 70f 140f
            //발목-무릎-엉덩이 무릎각도
            if (markResult[23 + side][25 + side][27 + side] == true) { //각도 판별
                iv5.setImageResource(R.drawable.squat_leg_green);
                resultPosture[3] = 2;
            } else {
                iv5.setImageResource(R.drawable.squat_leg_red);
                resultPosture[3] = 1;
            }
        } else {
            //여기에 비감지(회색)
            iv5.setImageResource(R.drawable.squat_leg_gray);
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public void getTimeIntent() {
        ...
        squatTargetCount = intent.getIntExtra("count", 2);
        squatAllTimer = (intentMinute*60) + intentSecond;
    }
    ...
    protected int getContentViewLayoutResId() {
        return R.layout.activity_squat;
    }
